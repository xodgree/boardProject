...
<%
	...
	// Article의 개수가 0보다 많은 경우, Article 목록을 가져옵니다.
	if(count > 0) {
...
%>
	<!-- Article의 개수가 0이 아닐 때, 실행되는 스크립트 -->
	<table class="w3-table-all" width="700">
		<tr class="w3-babypink" style="background-color:rgba(255, 0, 0, 0.4);">
			...
			<%
			// 위에서 DB로부터 얻는 Article을 담은 List를 순회하면서 Article에 담긴 정보를 보여줍니다.
			for(int i=0; i < articleList.size(); i++) {
				// articleList에 담긴 자료는 BoardDataBean입니다.
				// BoardDBBean의 getArticles() 메소드를 참고합니다.
				// articleList에 담긴 BoardDataBean에 인덱스 i로 접근합니다.
				// 캐스팅을 한 이유는 List에 담긴 자료형을 확정하기 위해서 입니다.
				BoardDataBean article = (BoardDataBean) articleList.get(i);
			%>
				<!-- 아래의 스크립트는 모두 BoarddataBean에 저장된 데이터(Article 데이터)를 보여줍니다. -->
				<tr height="30">
